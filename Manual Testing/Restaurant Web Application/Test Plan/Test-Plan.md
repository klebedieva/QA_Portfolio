...
The purpose of testing is to verify that the application's main user flows work correctly, that calculations and validations are accurate, and that users can complete reservations and orders without functional errors.
...
- Validate price, tax, delivery fee, and total calculations.
...
- Navigation to Home, Menu, Gallery, Contact, Reservation, and Cart
...
- Product name, image, description, category, dietary badges, and price
- Closing product details
- Adding an item to the cart from product details, if supported
...
Verify layout, labels, buttons, fields, visual states, and feedback messages.
...
Verify limits such as price ranges, quantities, dates, guest counts, and field lengths.

### End-to-End Testing

Verify complete flows such as:

- Browse menu → add item → cart → checkout → confirmation
...
- Backend order and reservation services
...
| Backend confirmation without visible admin access | Order persistence cannot be fully verified | Verify confirmation UI and request backend evidence if available |
...
- Order and reservation submission services are available in the test environment.
...
4. Update quantity and verify totals
5. Choose delivery or pickup
6. Enter customer information
7. Select date, time, and payment method
8. Accept terms and confirm the order
9. Verify order confirmation and order number
10. Submit a valid reservation
11. Submit a valid contact request
